...
\begin{itemize}
	...
	\item заполнения массива a\_tmp, трудоёмкость которого (\ref{for:ATMP_impr}):
	\item \begin{equation}
		\label{for:ATMP_impr}
		f_{a\_tmp} = 2 + K (2 + \frac{M}{2} \cdot 9);
	\end{equation}

	\item заполнения массива b\_tmp, трудоёмкость которого (\ref{for:BTMP_impr}):
	\item \begin{equation}
		\label{for:BTMP_impr}
		f_{b\_tmp} = 2 + K (2 + \frac{N}{2} \cdot 9);
	\end{equation}

	\item цикла заполнения для чётных размеров, трудоёмкость которого (\ref{for:cycle_impr}):
	\begin{equation}
		\label{for:cycle_impr}
		f_{cycle} = 2 + M \cdot (9 + N \cdot (7 + \frac{K}{2} \cdot 20));
	\end{equation}
	
	\item цикла, который дополнительно нужен для подсчета значений при нечетном размере матрицы, трудоемкость которого (\ref{for:last_impr}):
	\begin{equation}
		\label{for:last_impr}
		f_{last} = \begin{cases}
			2, & \text{чётная,}\\
			4 + M \cdot (8 + 8N), & \text{иначе.}
		\end{cases}
	\end{equation}
\end{itemize}
